...
Use the repository root commands documented in `docs/self-improvement.md`. `python scripts/research_day.py` runs propose → dry-evaluate → status and then stops for a human decision. Every action is appended to `experiments/agent-actions.jsonl`. `python agents/reviewer.py list` also writes `experiments/QUEUE.md`. `python agents/reviewer.py status` prints local queue counts without calling any API. `python scripts/serve_review.py` exposes the same queue in a local browser console. Approve/reject decisions are terminal.
